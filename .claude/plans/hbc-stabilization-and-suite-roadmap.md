...
- **Phase 4F (23 Feb 2026)**: Analytics Hub Dashboard BUILT — AnalyticsHubDashboardPage at `/` with 5 KPI cards (Active Projects, Pipeline Value, Win Rate, Safety Score, On-Time %), 6 interactive ECharts (pipeline funnel, status treemap, win rate trend, resource heatmap, labor rate benchmark, material cost index), recent activity feed (15 mock entries), role-gated workspace quick links (RoleGate + FeatureGate), PowerBI embed placeholder. FunnelChart + TreemapChart registered. Feature flag: PowerBIIntegration (disabled). No new IDataService methods — composes from existing getLeads/getActiveProjects/getPortfolioSummary/getProcoreProjects/getBambooEmployees.
- **Phase 4F-precon (23 Feb 2026)**: Preconstruction + Estimating Dashboards BUILT — PreconDashboardPage at `/preconstruction` rebuilt with 4 KPI cards (Active Leads, Go/No-Go In Progress, Pipeline Value, Est. Win Rate), 3 ECharts (lead funnel by stage, win rate by PE with FL GC 28.7% benchmark line, post-bid autopsy trend with deterministic fallback), sub-hub quick links (BD, Estimating, IDS), PowerBI placeholder. EstimatingDashboardPage at `/preconstruction/estimating` rebuilt with 5 KPI cards (Total Estimates, Active Pursuits, Submitted, Pursuit Pipeline, Precon Engagements), 3 ECharts (award status donut with semantic colors, estimates by source bar, estimator workload grouped bar). Data hooks: `usePreconDashboardData` (4 parallel fetches: getLeads, getScorecards, getEstimatingRecords, getAllLossAutopsies) + `useEstimatingDashboardData` (3 parallel fetches: getEstimatingRecords, getCurrentPursuits, getPreconEngagements). Both elevated to 4.75/10 with Griffel makeStyles, responsive grids, hover elevation. No new IDataService methods. 688 tests passing.
- **Phase 4H (23 Feb 2026)**: Project Hub Module BUILT — Cross-cutting project workspace at `/project-hub/*` with `requireProject: true`. Hidden from AppLauncher + sidebar unless project selected. 10 accordion sidebar groups (flattened from plan hierarchy): Project Hub, Preconstruction, Project Manual, Startup & Closeout, QA/QC Program, Financial Forecasting, Schedule, Logs & Reports, Monthly Reports, Documents. 37 routes (1 layout + 36 pages). ProjectHubLayout with MessageBar "No Project Selected" banner. 36 fresh page components in `pages/project-hub/`. Feature flag: ProjectHubWorkspace. IWorkspaceConfig extended with `requireProject?: boolean`. AppLauncher filters workspaces. ContextualSidebar quick-link added. 2 Storybook stories (Dashboard + Settings). 136 total app routes. 688 tests passing.
