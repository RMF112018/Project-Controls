...
81. **Jest 29 `testEnvironment` must use explicit path to avoid SPFx v25 conflict** — SPFx 1.21.1 hoists `jest@25` to the root `node_modules/`, which shadows `jest@29`'s `jest-environment-node`. The `jest.config.js` in `packages/hbc-sp-services/` resolves this with `testEnvironment: path.resolve(__dirname, '../../node_modules/jest/node_modules/jest-environment-node')`. Do not change this to `'node'` — it will resolve to the wrong version and fail with `TypeError: Class extends value undefined is not a constructor or null`.
